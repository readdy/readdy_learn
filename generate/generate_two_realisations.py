...
def generate(n_timesteps, fname, n_a_particles=6000, n_b_particles=6000):
    ...
    sim.register_reaction_fusion("C+D->B", "C", "D", "B", .05, .4)
    ...
    with closing(io.File(fname, io.FileAction.CREATE, io.FileFlag.OVERWRITE)) as f:
        ...
        sim.run_scheme_readdy(True).with_reaction_scheduler("Gillespie").write_config_to_file(f).configure(time_step).run(n_timesteps)


if __name__ == '__main__':
    generate(50000, "two_realisations_1.h5", n_a_particles=6000, n_b_particles=6000)
    generate(50000, "two_realisations_2.h5", n_a_particles=2000, n_b_particles=8000)
