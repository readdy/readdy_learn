...
\begin{abstract}
The inner workings of a biological cell or a chemical reactor can
...
reaction network whereas non-sparse estimation detects many spurious processes.
...
reactions (creation), order-1 reactions (transitions
of other species into $s$, transitions of $s$ into other species, or annihilation),
...
\begin{figure}[h!]
\centering \includegraphics[width=\singlecolumnwidth]{1.pdf}
...
\begin{figure}
\centering\includegraphics[width=\singlecolumnwidth]{2.pdf}\caption{\label{fig:case-1-lsq}Concentration time series generated from integrating
...
\begin{figure*}
\centering \includegraphics[width=0.82\doublecolumnwidth]{3.pdf}
...
\begin{figure}
\centering \includegraphics[width=\singlecolumnwidth]{4.pdf}
...
\begin{figure}
\centering \includegraphics[width=\singlecolumnwidth]{5.pdf}
...
The response yielded by the chosen initial conditions is marked in Fig.~\ref{fig:mapk}a by vertical dashed lines.
The concatenated time series is a dataset of 300 frames in total.
We use the library $\Theta$ of ansatz reactions Tab.~\ref{tab:reaction-library-mapk}.
The hyperparameter $\alpha=6.6\times 10^{-9}$ was determined by shuffling the data and performing
15-fold cross validation. The estimated 
rate constants were obtained by solving the minimization problem (\ref{method:minimizationproblem}) 
...
ground truth model. However it also detects 12 spurious reaction 
processes ($\theta_{18}$ - $\theta_{29}$).
...
beneath a certain cutoff. In this case for example a cutoff of $\kappa=0.25$ would directly recover
...
\begin{figure}
    \centering
    \includegraphics[width=\singlecolumnwidth]{6.pdf}
...
(predator) is defined by the reaction network
\begin{align*}
...
\end{align*}
From this model we generated concentration time series data with $200$ frames 
which is displayed in Fig.~\ref{fig:lv}a. The library 
of ansatz reactions $\Theta$ is given in Tab.~\ref{tab:reaction-library-lv}. 
The hyperparameter $\alpha=2.7\times10^{-7}$ was determined by shuffling 
the data and performing 5-fold cross validation. The estimated 
rate constants were obtained by solving the minimization problem (\ref{method:minimizationproblem}) 
with $\lambda=1$. The results are depicted in Fig.~\ref{fig:lv}b.
Least-squares estimation detects all reactions of the ground truth model but also
two spurious processes ($\theta_{6}$ and $\theta_{7}$) with a higher rate than
the first two underlying processes ($\theta_{1}$ and $\theta_{2}$). Reactive SINDy recovers
the true reaction network with minor deviations in rate constants. As in 
Sec.~\ref{sec:mapk}, considering the $L_1$ norm of the relative distance to the ground truth
...
\begin{figure}
    \centering
    \includegraphics[width=\singlecolumnwidth]{7.pdf}
...
\centering\includegraphics[width=\singlecolumnwidth]{8.pdf}\caption{\label{fig:reaction-rates-sorted}Reaction rates sorted by their magnitude
