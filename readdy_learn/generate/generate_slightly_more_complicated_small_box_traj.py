...
def generate(n_timesteps, fname):
    ...
    def callback(n):
        global count
        print("currently %s particles(%s%%)" % (n, (count*5000.)/n_timesteps))
        count = count + 1.
    # handle = sim.register_observable_trajectory(1)
    sim.register_observable_n_particles(5000, [], callback)
    ...
    with closing(io.File(fname, io.FileAction.CREATE, io.FileFlag.OVERWRITE)) as f:
        ...
        n_particles_handle.enable_write_to_file(f, u"n_particles", int(5000))
...
if __name__ == '__main__':
    generate(500000, "slightly_more_complicated_small_box_2_500000.h5")
